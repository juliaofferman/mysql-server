...
class KMEANS_API PartitionerWrapper {
 public:
  ...
  virtual void tokenize(DatapointWrapper *dp, std::vector<int32_t> *parts,
                        int32_t num_parts) = 0;
...
KMEANS_API std::unique_ptr<DatapointWrapper> CreateQuantizedDatapoint(
    const int8_t *values, size_t size);

KMEANS_API float DenseDotProduct(DatapointWrapper *q, DatapointWrapper *dp);
