...
struct Partitioner {
  ...
  template <typename T>
  index_err_t tokenize(const T &dp, Partitions *result,
                       int32_t num_parts = -1) {
    if (!result) return SUCCESS;
    auto span = dp.values_span();
    auto dp_wrap =
        kmeans_wrapper::CreateDatapointFromPtr(span.data(), span.size());
    if (impl_) {
      if (num_parts > 0) {
        impl_->tokenize(dp_wrap.get(), result, num_parts);
      } else {
        impl_->tokenize(dp_wrap.get(), result);
      }
    } else {
      result->push_back(0);
    }
...
using QuantizedDatapoint = std::unique_ptr<kmeans_wrapper::DatapointWrapper>;
using QuantizedDatapointPtr = kmeans_wrapper::DatapointWrapper*;
...
using AsymmetricHasherConfig = research_scann::AsymmetricHasherConfig;
using ProjectionConfig = research_scann::ProjectionConfig;
